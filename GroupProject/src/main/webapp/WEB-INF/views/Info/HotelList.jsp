...
<body>
	...
	<!-- 본문내용 자리입니다. -->
		...
		<div class="row mb-100 mt-100">
			<div class="col-12">
				<form class="user" action="hotelSearchList" method="post" enctype="multipart/form-data" >
						시/도  
						<select id="metropolis" name="metropolis" onchange="changeAreaSelect(this)">
		...
		<table class=" table table-layout: fixed text-center" id="dataTable" cellspacing="0">
			...
			<thead class="table-dark">
				<tr class="text-center">
					...
					<th width="208px">애견호텔 이름</th>
					<th width="604px">주소</th>
			...
			<tbody class="table-hover" id="mywalkList">
				<c:forEach items="${show_HotelList}" var="hotel">
					<tr class="table-body-row">
						<c:set var="codename"	value="${fn:substring(report.recomment_boardcode,0,2)}" />
						<c:set var="codenumber" value="${fn:substring(report.recomment_code,2,6)}" />
						<td class="product-remove" >${hotel.ho_name }</td><!-- 호텔 이름 -->
						<td class="product-quantity">${hotel.ho_addr }</td> <!-- 주소 -->
						<td class="product-quantity">${hotel.ho_tel }</td> <!-- 전화번호 -->
						<c:if test="${sessionScope.loginId eq 'ICIAadmin'}">
							<td class="product-quantity">
								<button onclick="viewHotelInfo(this,'${hotel.ho_code}')">
									수정하기
								</button>
							</td>
						</c:if>
	...
	<!-- adminModifyHotelModal Modal -->
	<div class="modal fade" id="adminModifyHotelModal" tabindex="-1" role="dialog"
		aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document" style="max-width: 800px;">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title text-gray-900 text-primary font-weight-bold"
							id="exampleModalLabel">호텔 정보 수정</h5>
					...
					<div class="modal-body">
						<div class="col-12">
							<div class="row">
									...
									<div class = "col-12" style="margin-bottom : 15.7px;">
										<label>호텔 주소</label> 
										<input id="ho_addr" name="ho_addr" type="text"  style="width: 500px;">
									</div>
									<div class = "col-12" style="margin-bottom : 15.7px;">
										<div style="width:70%;">
											<label>호텔 전화번호</label> 
	...
	function mywalkSearchList(){
		...
		$.ajax({
			...
			success : function (result){
  			...
  			function viewHotelInfo(selObj,ho_code) {
  				console.log("ho_code = " + ho_code);
  		
  				 // ajax로 호텔 모달창 열기
  				 $.ajax({
  					type : "get",
  					url : "adminViewHotelInfo",
  				...
  				$("#adminModifyHotelModal").modal('show');
  			}
  			...
  			function modifyHotel(){
  				...
  				//ajax 극장 정보 수정
  				$.ajax({
  					url : "adminModifyHotel",
  					type : "get",
  					data : { "ho_code" : ho_code, "ho_addr" : ho_addr, "ho_name" : ho_name, "ho_tel" : ho_tel, "ho_state" : ho_state},
  					async: false,
  					success: function(result){
  						console.log(result);
  						htInfoVal.ho_code = ho_code;
  						htInfoVal.ho_name = ho_name;
  						htInfoVal.ho_addr = ho_addr;
  						htInfoVal.ho_tel = ho_tel;
  						htInfoVal.ho_state = ho_state;
  						alert("정보가 수정 되었습니다.")
  						location.reload();
  					}
  				});
  			...
  			$("#adminModifyHotelModal").on('hide.bs.modal', function(){
  				/* alert("모달창 닫히기 전!"); */
  			});		

  			$("#adminModifyHotelModal").on('hidden.bs.modal', function(){
  				/* alert("모달창 닫힌 후!"); */
  			});
  			
  			$("#adminModifyHotelModal").on('show.bs.modal', function(){
  				/* alert("모달창 열리기 전!"); */
  			});		
  			
  			$("#adminModifyHotelModal").on('shown.bs.modal', function(){
  				/* alert("모달창 열린 후!"); */
  			});
