					...
					<div class="col-lg-12">
						<div class="card ">
											...
											<c:otherwise>
												...
												<c:choose>
													<c:when test="${walkBoardInfo.walk_writer == sessionScope.loginId}">
														<div style="float:right;">
															<a  class="cart-btn" style="width:120px; text-align:center;" onclick="crewWalkViewModifyPage()">수정</a>
							...
							<div class="card-body" style="padding:35px;">
								...
								<br>
								<h5 class="mb-0" style="margin-top:15px;">
									댓글목록	
								</h5>
									<br><br>
...
function recommentView(){
	...
	$.ajax({
		...
		success : function (result) {
			...
			output += "'); width:120px; height: 60px; background-size:cover; background-position:center; border-radius:35px;\"></div></div>"
			+"<div class=\"col-10\" style=\"padding-left:0px;\"><div class=\"row\"><div class=\"col-12\">"
...
function insertReport(RCcode){
	
	
	$.ajax({
		type : "get",
		url : "walkinsertReport_ajax",
		data : { "code" : RCcode,"boardcode" : '${walkBoardInfo.walk_code}' },
		async : false,
		success : function (result) {
			console.log(result);
		}
...
function walkDelState(){
	Swal.fire({
		...
		// 만약 모달창에서 confirm 버튼을 눌렀다면 //처리시작
			...
			location.href="crewWalkDelState?boardCode="+'${walkBoardInfo.walk_code}';
		}
...
function crewWalkViewModifyPage(){
	location.href="crewWalkViewModifyPage?boardCode="+'${walkBoardInfo.walk_code}';
}
...
function walkViewJoin(){
	...
	Swal.fire({
		...
		// 만약 모달창에서 confirm 버튼을 눌렀다면 //처리시작
			...
			location.href="crewWalkViewJoin?boardCode="+'${walkBoardInfo.walk_code}';
		}
