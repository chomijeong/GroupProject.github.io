...
<body>
	...
	로그인된 세션아이디 : ${sessionScope.loginId}로그인된 크루코드 : ${sessionScope.crewCode}
	...
	if ((key === 'Enter' && !event.shiftKey) || (key === 13 && key != 16)) {
    	event.preventDefault();
    	 ...
    	 if(chatContent.trim().length >0){
    		...
    		output += "<div style=\"margin:30px;\"><div class=\" d-flex justify-content-end align-items-end \">"
				+"<span style=\"font-size:15px; margin-right:10px; font-weight:400;\">나</span>"; 
			if(${sesseionScope.loginProfile !=null}){
				output+= "<div style=\"width:50px; height:50px; background-size:cover; border-radius:3px; background-position:center; background-image:url('${pageContext.request.contextPath}/resources/fileUpload/${sesseionScope.loginProfile}');\"></div>";
