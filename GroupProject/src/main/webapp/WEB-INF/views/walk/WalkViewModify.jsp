...
/*휠명령되는 a태그 스타일*/
#wheelST,#wheelST2{
background-image: url('${pageContext.request.contextPath}/resources/myupload/wheel2.png');
background-repeat: no-repeat;
background-size: 30px;
background-position: right;
}
#clickST{
background-image: url('${pageContext.request.contextPath}/resources/myupload/click2.png');
background-repeat: no-repeat;
background-size: 30px;
background-position: right;
}
...
<body>
					...
					<div class="col-lg-6">
						<div class="card ">
							<div class="card-header">
								...
								<div style="display:inline-block; color:rgb(0,0,0,0.5);">작성일시: ${walkBoardInfo.walk_date}</div>
								</h5>
							</div>
							...
							<div class="card-body">
								<div class="billing-address-form row">
									<div class="col-12">
										...
										<div  id="mapOriginDiv">
										<p>
											<input type="text" readonly id="loadAddr" value="${walkBoardInfo.walk_startaddr}">
										</p>
										<p>
											<input type="text" readonly id="totalDistanceOrigin" value="총거리: 도보: ">
										</p>
										
										<p>
											<div class="card" >
												<div class="card-header text-center">
													<div id="toggleNew">기존 경로입니다</div>
												</div>
											
												<div class="card-body" style=" margin:0px; padding: 0px;">
													<div  id="mapOrigin" style=" background-color: gray; width: 100%; height: 350px; border-radius: 3px; ">											
													</div>
										...
										<p>
											<div>
												<a class="cart-btn text-center" style="width: 100%;" onclick="togglaNewMap('new')">새 경로 만들기</a>
											</div>
										</p>
										</div>
									...
									<!-- ---------------- -->
										...
										<div id="mapNewDiv">
										<p >
											<input type="text" readonly id="startAddr" value="출발지: ">
										</p>
										<p  >
											<input type="text" style="width: 60%;" id="inputword"><a style="width: 40%;" id="inputbtn" class="cart-btn text-center" onclick="startMap()">주소 검색하기</a>
										</p>
										...
										<p>
											<div class="card" ">
												<div class="card-header text-center">
													<div>클릭으로 좌표를, 우클릭으로 종료합니다.</div>
												</div>
											
												<div class="card-body" style=" margin:0px;padding: 0px;">
													<div onclick="toggleOverlay()" id="mapNew" style=" background-color: gray; width: 100%; height: 350px; border-radius: 3px; ">											
													</div>
										...
										<p>
											<div >
												<div class="d-flex justify-content-between">
													<a class="cart-btn text-center" style="width: 80%;"  onclick="togglaNewMap('origin')">이전 경로 불러오기</a>
													<a class="cart-btn text-center" onclick="mapReset()"><i style="margin-right:0px;"class="fa-solid fa-arrows-rotate"></i></a>
												</div>
											</div>
										</p>
										</div>
							...
							<div class="card-body">
								<div class="billing-address-form">
													...
													<div style="width: 100%; margin-left: 17px;">
														...
														<p>
															<div class="d-flex justify-content-between">
								        						...
								        						<a onclick="checkDateData()" id="clickSubmit"class="inputbtn" style="width:45%; height:111px; 
						        	...
						        	<p id = "people">
										...
										class="inputbtnOff inputbtnOffAdd" id="people2" onclick="inputbtnSTtoggle2(this)">2명</a><a style="border-radius:0px" 
										class="inputbtnOff inputbtnOffAdd" id="people3" onclick="inputbtnSTtoggle2(this)">3명</a><a style="border-radius:0px" 
										class="inputbtnOff inputbtnOffAdd" id="people4" onclick="inputbtnSTtoggle2(this)">4명</a><a style="border-radius:0px 3px 3px 0px;"
										class="inputbtnOff inputbtnOffAdd" id="people5" onclick="inputbtnSTtoggle2(this)">5명</a>
									</p>
						        	<p>
						        		<div class="row">
						        			<div class= "col">
						        				<a id="dogS" class="inputbtnOff" style="width:100%" onclick="inputbtnSTtoggle(this,0)">10kg미만<br>소형견</a>
						        			</div>
						        			<div class= "col">
						        				<a id="dogM" class="inputbtnOff" style="width:100%" onclick="inputbtnSTtoggle(this,10)">10kg~25kg<br>중형견</a>
						        			</div>
						        			<div class= "col">
						        				<a id="dogL" class="inputbtnOff" style="width:100%" onclick="inputbtnSTtoggle(this,25)">25kg초과<br>대형견</a>
						        			</div>
										</div>
									</p>
									<p>
									
										<input id="title" type="text" placeholder="제목" value="${walkBoardInfo.walk_title}">
									
									</p>
						        	<p>
										<textarea name="bill" id="bill" cols="30" rows="10"
											placeholder="모집 상세 안내 사항" >${walkBoardInfo.walk_content}</textarea>
									</p>
	...
	<div style="display:none">
		<form id ="finalForm" action="walkViewModify" method="POST">
			<!-- 산책글코드 -->
			<input name="walk_code" type="text" value="${walkBoardInfo.walk_code}">
			<!-- 작성자아이디 -->
...
<script type="text/javascript">
//기존 마감시간정보 출력
var memory = '${walkBoardInfo.walk_time}';
console.log("memory"+memory);
var memoryDate = memory.split(" ")[0];
console.log("memoryDate"+memoryDate);
 

$( "#datepicker" ).datepicker( "setDate", "22/07/2022");  
var memoryHour = Number(memory.split(" ")[1].split(":")[0]); 
if(memoryHour==12){
	$("#clickST").text("오후");
}else if(memoryHour>12){
	memoryHour -= 12;
}
$("#wheelST").text(memoryHour+"시");
var memoryMin = Number(memory.split(" ")[1].split(":")[1]); 
if(memoryMin==0){
	$("#wheelST2").text("정각");
}else{
	$("#wheelST2").text(memoryMin+"분");
}

//인원수 입력
memory = '${walkBoardInfo.walk_people}';
memory = memory.replace("명","");
$("#people"+memory).toggleClass("inputbtnOff").toggleClass("inputbtn");
var formPeople = memory; //4
//강아지 제한 입력
memory= '${walkBoardInfo.walk_bodytype}';
var dogrow = memory.split("/");

if(dogrow[0] !="----"){
	$("#dogS").toggleClass("inputbtnOff").toggleClass("inputbtn");
}if(dogrow[1] !="----"){
	$("#dogM").toggleClass("inputbtnOff").toggleClass("inputbtn");
}if(dogrow[2] !="----"){
	$("#dogL").toggleClass("inputbtnOff").toggleClass("inputbtn");
}


$( document ).ready(function() {
	$("#mapNewDiv").css("display","none");
});

function togglaNewMap(word){
	if(word=="new"){
		$("#mapNewDiv").css("display","");
		$("#mapOriginDiv").css("display","none");
	}
	if(word=="origin"){
		$("#mapOriginDiv").css("display","");
		$("#mapNewDiv").css("display","none");
		mapReset();
		$("#inputword").val("");
...
//근본주소 (예 인천 서구)저장합니다.
var firststartAddr= '${walkBoardInfo.walk_startaddr}';
firststartAddr=firststartAddr.split(" ")[1]+" "+firststartAddr.split(" ")[2];
$("#startAddr").val("출발지: "+firststartAddr);
...
var beforeInputaddr = firststartAddr; //회원의 지역+ 검색지역 두번돌려야해서 초기 주소만을 담을 전역변수입니다.  
...
var mapContainer = document.getElementById('mapNew'), //지도를 표시할 div 
    mapOption = {
...
var ps = new kakao.maps.services.Places(); 
	...
	ps.keywordSearch(beforeInputaddr, placesSearchCB1); 
	...
	function placesSearchCB (data, status, pagination) {
    	if (status === kakao.maps.services.Status.OK) {
        	...
        	for(var i =0; i<data.length; i++){
         	...
         	console.log("검색결과 후 지도레벨:"+getLevel);
         	if(getLevel<=3){
...
var clickLine=null; // 마우스로 클릭한 좌표로 그려질 선 객체입니다.
...
var clickLine1 = new kakao.maps.Polyline({
		strokeWeight: 3, // 선의 두께입니다 
 	strokeColor: '#db4040', // 선의 색깔입니다
 	strokeOpacity: 1, // 선의 불투명도입니다 0에서 1 사이값이며 0에 가까울수록 투명합니다
 	strokeStyle: 'solid' // 선의 스타일입니다
...
//서버에서 가져온 좌표를 카카오 맵에서 사용할 수 있도록 변환하는 작업
	...
	var mapContainer1  = document.getElementById('mapOrigin'), // 지도를 표시할 div.
		mapOption1  = { 
	...
	//지도 마무리 설정
map1.setDraggable(false);//드래그를 막는다
map1.setZoomable(false);//줌스크롤을 막는다

map1.setBounds(customBounds);//여러좌표들을 다 담는 화면이동을 한다(확대레벨 바뀌어버림)
console.log("customBounds내용:"+customBounds);
console.log("최종지도레벨"+map.getLevel());
//출발 마커 생성 전 이미지 옵션입니다.
var startSrc1 = 'https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/red_b.png', // 출발 마커이미지의 주소입니다(카카오에서 쓰는 아이콘주소임)    
startSize1 = new kakao.maps.Size(50, 45), // 출발 마커이미지의 크기입니다 
startOption1 = { 
    offset: new kakao.maps.Point(15, 43) // 출발 마커이미지에서 마커의 좌표에 일치시킬 좌표를 설정합니다 (기본값은 이미지의 가운데 아래입니다)
};

//출발 마커 이미지를 생성합니다
var startImage1 = new kakao.maps.MarkerImage(startSrc1, startSize1, startOption1);

//출발지 마커를 정의합니다.
var startMarker1 = new kakao.maps.Marker({
    draggable: false, // 출발 마커가 드래그 가능하도록 설정합니다
    image: startImage1 // 출발 마커이미지를 설정합니다
});

//도착 마커 생성 전 이미지 옵션입니다.
var arriveSrc1  = 'https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/blue_b.png', // 출발 마커이미지의 주소입니다(카카오에서 쓰는 아이콘주소임)    
arriveSize1 = new kakao.maps.Size(50, 45), // 도착 마커이미지의 크기입니다 
arriveOption1  = { 
    offset: new kakao.maps.Point(15, 43) // 도착 마커이미지에서 마커의 좌표에 일치시킬 좌표를 설정합니다 (기본값은 이미지의 가운데 아래입니다)
};
//도착 마커 이미지를 생성합니다
var arriveImage1 = new kakao.maps.MarkerImage(arriveSrc1, arriveSize1, arriveOption1);

//도착 마커를 정의합니다.
var arriveMarker1 = new kakao.maps.Marker({
    draggable: false, // 도착 마커가 드래그 x
    image: arriveImage1 // 도착 마커이미지를 설정합니다
});


//출발마커 찍기
startMarker1.setPosition(customPath[0]);
startMarker1.setMap(map1);
//도착마커 찍기
arriveMarker1.setPosition(customPath[customPath.length-1]);
arriveMarker1.setMap(map1);

clickLine1.setPath(customPath);
clickLine1.setMap(map1);

//선의 길이 총 길이와, 걷는 시간을 표시하는 output출력내용입니다.
var distance1 = Math.round(clickLine1.getLength());
var walkkTime1 = distance1 / 67 | 0;

$("#totalDistanceOrigin").val("총 거리: "+distance1+"m, 도보: "+walkkTime1+"분");
...
//기존날짜 입력
	var memory2 = '${walkBoardInfo.walk_time}';
	var inputday = new Date(memory2);
	console.log("inputday"+inputday);
	$( "#datepicker" ).datepicker( "setDate", inputday);  
...
var memberHaveDogWeight = '${memberDto.dogsWeightCode}';
...
/*inputbtnOff는 흰색바탕에 회색글자, inputbtn는 주황박스*/
function inputbtnSTtoggle(aObj,Num){
	
	if(Num == 0){
		
		if(mHDW_S=="0010"){
			Swal.fire("회원님에 등록된 강아지몸무게의 그룹은 해제할 수 없습니다.");
			
		}else{
			if($(aObj).hasClass("inputbtnOff")){
				$(aObj).toggleClass("inputbtnOff").toggleClass("inputbtn");
				fdw1 = "0010";
			}else{
				$(aObj).toggleClass("inputbtnOff").toggleClass("inputbtn");
				fdw1 = "----";
...
function sendFormData(){
	...
	//지도 그리는 중 체크 
	if(drawingFlag == true){
		console.log("갈정보 체크:"+drawingFlag);
		finalcheck = false;
	...
	if(formWeather.length ==0  ){
		console.log("formWeather.lenght:"+formWeather);
		finalcheck = false;
		Swal.fire("날씨를 검색해주세요");
		return null;
	}
	...
	//지도좌표정보체크
	if(row.length>=2){ //그리는상태가 아니고, 지도좌표가 2개이상일경우 (지도 업데이트 가능할 경우)
	...
	console.log("인원수정보:"+Number(formPeople.replace("명","")));
